        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{jito_url}/sendBundle", json=body, headers=headers) as bresp:
                    ...
                    out_amt = Decimal(str(swap_data.get("swap", {}).get("outAmount", 0)))
                    in_amt = Decimal(str(swap_data.get("swap", {}).get("inAmount", 1)))
...
async def mirror_sell(
    ...
    **kwargs
    ...
    """
    Mirror a sell: swap position.token into SOL using position.size.
    Returns (signature, filled_price).
    """
